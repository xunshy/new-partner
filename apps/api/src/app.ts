...
export async function createApp(databasePath?: string) {
  ...
  app.post('/api/partners', async c => {
    ...
    return write(db, async tx => {
      ...
      const { rows } = await tx.execute({ sql: 'SELECT last_generated FROM sessions WHERE id = ?', args: [owner] });
      ...
      // The generated partner and its lifetime count commit together across all instances.
      await tx.execute({ sql: "DELETE FROM events WHERE partner_id IN (SELECT id FROM partners WHERE owner = ? AND json_extract(data, '$.saved') = 0)", args: [owner] });
      await tx.execute({ sql: "DELETE FROM partners WHERE owner = ? AND json_extract(data, '$.saved') = 0", args: [owner] });
      await tx.execute({ sql: 'INSERT INTO partners (id, owner, data) VALUES (?, ?, ?)', args: [partner.id, owner, JSON.stringify(partner)] });
      await tx.execute({ sql: 'UPDATE generation_counts SET count = count + 1 WHERE rarity = ?', args: [partner.rarity] });
      await tx.execute({ sql: 'UPDATE sessions SET last_generated = ? WHERE id = ?', args: [Date.now(), owner] });
      return c.json({ partner }, 201);
    });
  });
  app.post('/api/partners/:id/save', c => write(db, async tx => {
    const partner = await read(tx, c.req.param('id'), c.get('owner'));
    if (!partner) return c.json({ error: '这个对象已离开当前会话，请重新生成' }, 404);
    partner.saved = true; await update(tx, partner);
  ...
  app.patch('/api/partners/:id', async c => {
    ...
    return write(db, async tx => {
      ...
      partner.name = parsed.data.name; await update(tx, partner);
      return c.json({ partner });
    });
  });
  app.delete('/api/partners/:id', c => write(db, async tx => {
    const partner = await read(tx, c.req.param('id'), c.get('owner'));
    if (!partner) return c.json({ error: '找不到这个对象' }, 404);
    await tx.execute({ sql: 'DELETE FROM events WHERE partner_id = ?', args: [partner.id] });
    await tx.execute({ sql: 'DELETE FROM partners WHERE id = ?', args: [partner.id] });
  ...
  app.post('/api/partners/:id/interactions', async c => {
    ...
    return write(db, async tx => {
      ...
      const { rows } = await tx.execute({ sql: 'SELECT created_at FROM events WHERE partner_id = ? ORDER BY created_at DESC LIMIT 1', args: [partner.id] });
      const now = Date.now();
      ...
      await update(tx, partner);
      await tx.execute({ sql: 'INSERT INTO events (id, partner_id, created_at, data) VALUES (?, ?, ?, ?)', args: [event.id, partner.id, now, JSON.stringify(event)] });
